...
def signup():
    ...
    if request.method == "POST":
        ...
        if registered_user:
            flash(
                f"Sorry, but {request.form.get('username')} has already been\
                taken.")
        ...
        if len(request.form.get("password")) < 5 or len(request.form.get(
                "password")) > 15:
        ...
        shapes = [
            "squares/", "isogrids/", "spaceinvaders/",
            "labs/isogrids/hexa/", "labs/isogrids/hexa16/"]
        theme = [
        ...
        # Create browser session for new user and redirect to their
        # profile page
...
def login():
    ...
    if request.method == "POST":
        ...
        # Check that the user exists, and that the username and the
